...
import type { Event, StaffMember } from "@/lib/types";
      ...
      <div className="max-w-2xl w-full">
        <div className="mb-8 text-center">
          ...
          <p className="text-slate-500 mt-2 max-w-lg mx-auto">
            ...
            To run offline in the field, install the <strong>desktop companion</strong> on your laptop.
          </p>
        </div>
        ...
        <div className="space-y-4 mb-8">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-5 flex gap-4 shadow-sm">
            <div className="w-8 h-8 rounded-full bg-rose-600 text-white flex items-center justify-center font-bold text-sm shrink-0 mt-0.5">1</div>
            <div className="flex-1">
              <h3 className="font-bold text-slate-900 mb-1">Download the Event Med AI installer</h3>
              ...
              <p className="text-xs text-slate-400 mt-1.5">
                Grab <code className="bg-slate-100 px-1 rounded">Event-Med-AI_x64-setup.exe</code> (Windows, ~40 MB).
          ...
          {/* Step 2 */}
          <div className="bg-white border border-slate-200 rounded-2xl p-5 flex gap-4 shadow-sm">
            <div className="w-8 h-8 rounded-full bg-rose-600 text-white flex items-center justify-center font-bold text-sm shrink-0 mt-0.5">2</div>
            <div className="flex-1">
              ...
              <p className="text-xs text-slate-500 mt-1 leading-relaxed">
                ...
                After install completes, open <strong>Event Med AI</strong> from your Start Menu.
              </p>
              <p className="text-xs text-slate-500 mt-2 leading-relaxed">
                The app will guide you through installing <a href="https://ollama.com/download" target="_blank" rel="noreferrer" className="text-rose-600 font-semibold hover:underline">Ollama</a> and downloading
...
export default function SetupChecklistPage() {
    ...
    const fetchData = async () => {
      try {
        const [s, e, c] = await Promise.all([
          fetchSetupStatus(),
          apiFetch<Event>("/setup/event-info"),
          apiFetch<StaffMember[]>("/staff")
        ]);
        if (cancelled) return;
        setStatus(s);
        setEvent(e);
        setStaffCount(c.length);
        setFetchError(null);
      } catch (err) {
        if (cancelled) return;
  ...
  const handleReset = async () => {
    if (!confirm("This will clear all demo data and logs. Are you sure?")) return;
    setIsResetting(true);
    try {
      ...
      {fetchError && (
        <div className="mt-6 max-w-md text-center">
          ...
          <p className="text-xs text-slate-500 mt-3">
            The Event Med AI desktop app starts a small server on
          ...
          <button
            onClick={async () => {
              try {
                ...
                alert("Logs live in %APPDATA%\\EventMedAI\\logs\\ (Windows) or ~/Library/Application Support/EventMedAI/logs (Mac).");
              }
            }}
  ...
  const steps = [
    {
      ...
      action: status.ollama_installed ? null : (
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="inline-flex items-center gap-1.5 text-rose-600 font-semibold text-sm hover:underline"
        >
      ...
      action: status.model_ready ? null : (
        pulling ? (
          ...
          <button
            ...
            className="text-rose-600 font-semibold text-sm hover:underline disabled:text-slate-300"
          >
    ...
    },
    {
      ...
      desc: `Total members: ${staffCount}`,
      status: staffCount > 0 ? "done" : "active",
      required: false,
      action: (
    ...
    },
    {
      id: "event",
      title: "Event Details",
      desc: event?.name ?? "My Event",
      status: event && event.name !== "My Event" ? "done" : "active",
      required: false,
      action: (
        <button
          onClick={() => {
            const name = prompt("Enter event name:", event?.name);
            if (name) {
              apiFetch("/setup/event-info", {
                method: "POST",
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6 font-sans">
      <div className="max-w-xl w-full">
        ...
        <button
          ...
          disabled={finishing}
          className="w-full py-4 bg-rose-600 hover:bg-rose-700 disabled:opacity-60 text-white rounded-2xl font-bold shadow-xl shadow-rose-200 transition-all flex items-center justify-center gap-2"
        >
          {finishing ? <Loader2 className="animate-spin" size={20} /> : <ArrowRight size={20} />}
          {finishing ? "Launching…" : "Launch Event Med AI"}
        </button>
