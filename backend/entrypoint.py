...
def _resolve_data_dir() -> Path:
    if getattr(sys, "frozen", False):
        ...
        if sys.platform == "darwin":
            base = Path.home() / "Library" / "Application Support" / "EventMedAI"
        elif sys.platform == "win32":
            base = Path(os.environ.get("APPDATA", str(Path.home()))) / "EventMedAI"
        else:
            base = Path.home() / ".local" / "share" / "EventMedAI"
    else:
...
def main() -> None:
    ...
    os.environ.setdefault("EVENT_MED_DATA_DIR", str(data_dir))
    ...
    uvicorn.run(
        app,
        host="127.0.0.1",
        port=int(os.environ.get("EVENT_MED_PORT", "8000")),
        log_level="info",
    )
